 ...
 * @param time number 显示时长，默认1500ms
 */
function wxMsg(msg,time){
    time = time ? parseInt(time)/1000 : 1.5;
    layer.open({
        content: msg,
        style: 'background-color:#646567;color:#fff;border:none;',
        shade: false,
        time: time
    });
}
 ...
 * 类confirm弹框
 * @param msg string 提示内容
 * @param yesFuc function 点击确定的回调函数
 * @param noFuc function 点击取消的回调函数
 */
function wxConfirm(msg,yesFuc,noFuc){
    layer.open({
        content: msg,
        btn: ['确认', '取消'],
        shadeClose: false,
 ...
 */
function wxLoadShow(){
    $('body').eq(0).append('<div class="layerload"></div>');
}
function wxLoadHide(){
    $('.layerload').remove();
}